---
title: "Developing Front End Apps with React - Week 2"
image: "cover.png"
date: 2023-03-28T22:30:47Z
lastmod: 2023-03-28T22:30:48Z
...
Can only be sued in class components natively, in function componets opt-in to useState hook to include stateful features

Can reuse the component by giving it an abilty to receive data as input from the parent component

Cannot be accessd or modified outside of the component

Gives ability to receive data from the parent component in the form of props

Components can create and manage their own data with state

Components receive data from outside with prop

Used for managing data

Used to pass data

Data can be modified by its own component but is private

Data is read-only and cannot be modified by a component that is receiving it from outside

Can modifiy state with the setState() method

Can only be passed from parent component to child (unidirectional flow)
...
Component removal When a component is unmounted, the componentWillUnmount() method is called.
...
HTTP: GET(get remote data), POST(add remote data), UPDATE(change remote data), DELETE. Done by promise server call.
